...
contract SymbiosisFixtool {
    function decodeBridgeData(bytes memory data) public pure returns (bytes4, BridgeData memory, bytes memory) {
        bytes memory dataWithoutSelector = ThetaV2Util.slice(data, 4, data.length - 4);
        (BridgeData memory bridgeData, bytes memory _symBiosisData) = abi.decode(dataWithoutSelector, (BridgeData, bytes));
        return (bytes4(data), bridgeData, _symBiosisData);
    }

    function fixAmountBridgeData(bytes memory data, uint256 newAmount) public pure returns (bytes memory) {
        (bytes4 fs, BridgeData memory bridgeData, bytes memory _symBiosisData) = decodeBridgeData(data);
        bridgeData.amount = newAmount;
        return abi.encodeWithSelector(fs, bridgeData, _symBiosisData);
    }
