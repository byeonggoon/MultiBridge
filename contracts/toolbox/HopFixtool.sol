...
contract HopFixtool {
    function decodeBridgeData(bytes memory data) public pure returns (bytes4, BridgeData memory, HopData memory, bool) {
        bytes memory dataWithoutSelector = MultiBridgeV2Util.slice(data, 4, data.length - 4);
        (BridgeData memory bridgeData, HopData memory hopData, bool cctpCheck) = abi.decode(dataWithoutSelector, (BridgeData, HopData, bool));
        return (bytes4(data), bridgeData, hopData, cctpCheck);
    }

    function fixAmountBridgeData(bytes memory data, uint256 newAmount) public pure returns (bytes memory) {
        (bytes4 fs, BridgeData memory bridgeData, HopData memory hopData, bool cctpCheck) = decodeBridgeData(data);
        bridgeData.amount = newAmount;
        return abi.encodeWithSelector(fs, bridgeData, hopData, cctpCheck);
    }
